...
public class PeopleSupplyScene: SKScene{
    ...
    func clicked(node: SKSpriteNode, label: SKLabelNode){
        ...
        let jobsLabel = childNode(withName: "jobsLabel") as? SKLabelNode
//        node.run(.sequence[.fadeAlpha(to: 0.0, duration: 1.0), .move(to: CGPoint(x: (jobsLabel?.position.x)!, y: (jobsLabel?.position.y)!), duration: 1.0)])
//        [
        node.run(.group([.fadeAlpha(to: 0.0, duration: 1.0), .move(to: CGPoint(x: (jobsLabel?.position.x)!, y: (jobsLabel?.position.y)!), duration: 1.0)]))
    }
